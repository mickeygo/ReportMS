...
{
    /// <summary>
    /// AutoMapper 组件，用于弥补 v4.1.1.0 版本对 IDataReader 映射异常的处理
    /// 原档参考 https://github.com/AutoMapper/AutoMapper/blob/v3.3.1/src/AutoMapper/Mappers/DataReaderMapper.cs
    /// </summary>
    public class DataReaderMapper : IObjectMapper
    {
        ...
        private static readonly ConcurrentDictionary<BuilderKey, Build> _builderCache = new ConcurrentDictionary<BuilderKey, Build>();
        private static readonly ConcurrentDictionary<Type, CreateEnumerableAdapter> _enumerableAdapterCache = new ConcurrentDictionary<Type, CreateEnumerableAdapter>();

        public object Map(ResolutionContext context, IMappingEngineRunner mapper)
        {
            if (IsDataReader(context))
            {
                var useYieldReturn = mapper.ConfigurationProvider.DataReaderMapperYieldReturnEnabled;
        ...
        private class BuilderKey
        {
            ...
            public override int GetHashCode()
            {
                ...
                return _dataRecordNames.Aggregate(hash, (current, name) => current*37 + name.GetHashCode());
            }
            ...
            public override bool Equals(object obj)
            {
                ...
                return !_dataRecordNames.Where((t, i) => this._dataRecordNames[i] != builderKey._dataRecordNames[i]).Any();
            }
        }

        private class EnumerableAdapter<Item> : IEnumerable<Item>
        {
            readonly IEnumerable<Item> _items;
