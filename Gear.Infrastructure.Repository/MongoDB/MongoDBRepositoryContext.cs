...
using Gear.Infrastructure.Repository.MongoDB.Conventions;
...
namespace Gear.Infrastructure.Repository.MongoDB
{
    ...
    public class MongoDBRepositoryContext : RepositoryContext, IMongoDBRepositoryContext
    {
        ...
        {
            var conventionPack = new ConventionPack {new NamedIdMemberConvention("id", "Id", "ID", "iD")};
            if (autoGenerateID)
        ...
        public MongoCollection GetCollectionForType(Type type)
        {
            lock (syncObj)
            {
                ...
                var mongoCollection =
                    this.database.GetCollection(settings.MapTypeToCollectionName != null
                        ? settings.MapTypeToCollectionName(type)
                        : type.Name);
        ...
        public override void Commit()
        {
            lock (syncObj)
            {
                ...
                {
                    var collection = this.GetCollectionForType(newObj.GetType());
                    collection.Insert(newObj);
                }

                foreach (var modifiedObj in this.ModifiedCollection.Keys)
                {
                    var collection = this.GetCollectionForType(modifiedObj.GetType());
